...
export function CameraFeed({ config, detections, onFrame, isRunning }: CameraFeedProps) {
  ...
  // Handle IP camera / image URL: periodically fetch and draw to canvas
  useEffect(() => {
    ...
    const fetchFrame = () => {
      ...
      // Append cache buster for IP cameras
  ...
  const runDetectionLoop = useCallback(() => {
    ...
    // Detect every 1.5 seconds to avoid overwhelming TF.js
  ...
  if (error) {
    return (
      <div className="flex items-center justify-center h-64 bg-gray-900 rounded-xl border border-red-500/30">
